# -*- coding: utf-8 -*-
"""MGVAE_Model_MNIST_Soft_Clustering_Closed_Form.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1aPdF-0mLf_s9gaoXsfO4W-64g7ckvjBN

# **Mixtures of VAE Model using MNIST Dataset**
"""
...
def plot_confusion_matrix_posterior(y_true, y_pred, epoch):
    ...
    plt.savefig(f'Confusion_Matrix_Posterior_epoch_{epoch}.pdf', bbox_inches="tight")
    plt.close()

"""### **Log Probability of Normal Distribution**"""

PI=torch.from_numpy(np.asarray(np.pi))
def log_normal_diagonal_probability(X, mu, logvar):
  logprob= -0.5*torch.log(2.* PI) - 0.5 * logvar - 0.5 * torch.exp(-logvar) *(X-mu)**2
  return logprob
...
#     self.encoder_fc_1=nn.Linear(input_dim, encoder_hidden_dim_1)
#     self.encoder_fc_2=nn.Linear(encoder_hidden_dim_1, encoder_hidden_dim_2)
#     self.encoder_fc_3=nn.Linear(encoder_hidden_dim_2, encoder_hidden_dim_3)
...
#     mu_k=mu_layer.reshape(-1, num_components, latent_dim)
#     log_var_k=log_var_layer.reshape(-1, num_components, latent_dim)
...
#     self.decoder_fc_1=nn.Linear(latent_dim, decoder_hidden_dim_1)
#     self.decoder_fc_2=nn.Linear(decoder_hidden_dim_1, decoder_hidden_dim_2)
#     self.decoder_fc_3=nn.Linear(decoder_hidden_dim_2, decoder_hidden_dim_3)
#     self.decoder_final_layer=nn.Linear(decoder_hidden_dim_3, output_dim)
...
#     z=F.relu(z)
#     out=self.decoder_final_layer(z)
...
for epoch in range(num_epochs):
  ...
  for training_features, _ in train_loader:
    ...
    # log_p_z=torch.sum(p_z.log_prob(latent_z_k))
  ...
  with torch.no_grad():
    for test_features, test_labels in test_loader:
      ...
      ground_truth_labels.extend(test_labels.cpu().numpy())
      ...
      # log_p_z_test=torch.sum(p_z_test.log_prob(latent_z_k_test))
      ...
      # Max_Test_ELBO=log_lik_test[range(batch_size), best_k]
    ...
    # Latent Space
    latent_best=torch.cat(latent_best_z, dim=0)
    latent_best_cpu = latent_best.cpu().detach().numpy()
    latent_space=torch.cat(latent_space, dim=0)                                  # [N, KM]
    latent_space_cpu = latent_space.cpu().detach().numpy()
    Latent_embeddings.append(latent_space_cpu)
    Latent_embeddings_best.append(latent_best_cpu)

    # True Labels
    ground_truth_labels=torch.tensor(ground_truth_labels)
    ground_truth_labels_numpy=ground_truth_labels.numpy()
    ...
    for i in range(num_iterations):
      ...
      predicted_labels_gmm = gmm.fit_predict(latent_best_cpu)
      posterior_probabilities_gmm = gmm.predict_proba(latent_best_cpu)
    ...
    if (epoch + 1) == num_epochs:
        ...
        Reconstructed_Digits = Selected_Image.view(Selected_Image.size(0), 1, 28, 28)
        ...
        test_images = original_test_image.view(original_test_image.size(0), 1, 28, 28)
